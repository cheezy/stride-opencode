...
**Reviewer result (schema 1.7).** The `task-reviewer` agent emits a structured `reviewer_result` JSON block — `status`, `issue_counts`, `issues[]`, `acceptance_criteria[]`, `project_checks[]` (parsed from a project `CODE-REVIEW.md`), and the per-section `testing_strategy` / `patterns` / `pitfalls` / `security_considerations` verdicts (`security_considerations` is the fifth review_queue-scored field), plus an OPTIONAL top-level `behaviour_test_matrix` verdict (`{ status, note, rows[] }`) emitted only when the task supplied a `behaviour_test_matrix` and omitted entirely otherwise. Since schema 1.5 the `security_considerations` verdict may also carry an OPTIONAL nested `considerations[]` breakdown — one `{ consideration, status: mitigated | partial | unmitigated, evidence, note }` entry per listed consideration — populated only when the deep security review runs and absent otherwise (never required). Extract the first fenced ```json block per `stride-workflow` SKILL.md Step 6 and persist it verbatim into the completion payload (merged with the legacy summary fields) so the Stride review queue renders the full review. The schema is owned by `agents/task-reviewer.md`.
