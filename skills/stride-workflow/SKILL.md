...
Read this when you record the `reviewer` entry's cost figures, or when you read them back. The schema is the table above; this is what the two numbers mean once written down, and — the part that actually matters — what they cannot mean. **The limits are stated rather than papered over, because a figure trusted past its accuracy is worse than no figure at all.** The pair exists so the review phase's cost stops being invisible. It achieves that. It does **not** rank tasks by cost.
...
**(1) Wall-clock is not token cost, and the two disagree about which task was dearer.** Across the eight real dispatches measured when the key was introduced (W2130), tokens per second of wall-clock varied about **2.1×** by dispatch kind — reviewer rounds around 300–375 tok/s, specialist security dispatches around 455–625 tok/s. The figure therefore tracks a task's review *mix*, not its size. On the two real records available then, the pair ranked one task 20.3% more expensive when its token cost was in fact 1.4% *cheaper*: the order inverted, and the recorded gap was roughly fifteen times the real one. **Use the pair to see that a review phase ran long; never to conclude it was the costlier of two.**

**(2) The two keys measure different populations, so never divide one by the other.** `duration_ms` on this entry is an aggregate: the deep security-considerations review, the Step 6.5 session and Step 6.6 hardening all fold their wall-clock into it — that is what the fold rule in each of those steps instructs. `dispatch_count` counts only **this step's own subagent**, the `task-reviewer`, so it is typically the smaller number. On the two real records the duration covered four dispatches while the count reported two, and `duration_ms / dispatch_count` overstated the mean reviewer round by **40% and 52%**. The bias is not constant and the composition is recorded nowhere, so it cannot be divided back out. **`duration_ms / dispatch_count` is not a per-round figure — there is no per-round figure in this record at all. Do not compute one.**

**(3) A review-skipped task can carry a real review-phase cost and record none of it.** Step 6.5 is a step in its own right, gated only on task metadata and extension availability with **no reviewer precondition**, so the ordinary small-task route reaches it with review already skipped — a routine path here, not an exceptional one — and Step 6.6 follows from it on that same path. There the `reviewer` entry is the skip form: `dispatched: false`, no `duration_ms`, no `dispatch_count`. A task that ran a full exploratory session and drafted hardening checks is therefore byte-identical, within `workflow_steps`, to one whose review genuinely cost nothing. **This limit is narrower here than stride's, deliberately:** stride also counts its deep security review, whose gate likewise names no reviewer, but in this port that review is a sub-step *inside* Step 6 — which a small task skips entirely — so it cannot be reached on the review-skipped path and contributes nothing here. Those dispatches are recorded in `completion_notes` prose, which does not aggregate. **Absence of a cost figure is not evidence of absent cost.**

**(4) An omitted `dispatch_count` is ambiguous, and a reader must not impute a value.** It may mean an extension predating the key, or an oversight. (It should not mean a `1` the author knew and declined to write — the writing rule above says write it — but nothing in the record lets a reader tell those apart.) Imputing `1` rather than a higher value moved a sample mean by over 100%. **When aggregating, report the covered subset and its size instead of filling absences.** The ambiguity is widest during rollout, which is precisely when aggregating is most tempting.

**(5) It cannot separate a crashed re-dispatch from a genuine extra round — and in this port nothing else can either.** A `2` may be one round plus one crash, or two rounds. That is deliberate: the count measures *cost*, and a crashed dispatch really did spend its tokens. But it has a consequence worth stating plainly, because **a fully compliant `3` — two rounds plus a crash — reads like a breach of the two-round cap to anyone who knows that cap. It is not one.** Stride can settle such a question after the fact from a persisted round-count artifact. **This port persists nothing of the kind**: the round count is held in the session only, and the completion payload carries no round number anywhere. Round scoping in this port is **prose in the reviewer's invocation prompt** (Step 6, “What round two's invocation carries”), which nothing parses and which Step 6 expressly declines to make a structured field. So the distinction is unrecoverable here the moment the session ends. Two things follow, and they are the whole reason this limit is worth its space in a port that lacks the artifact: **never read a cap breach out of `dispatch_count` alone**, and when a count exceeds the rounds you actually ran, **say in `completion_notes` what it meant** — that sentence is the only record there will ever be.

**(6) Nothing validates the value on the way in — not this port, not the server — so the first consumer must guard it.** **This port validates no part of `workflow_steps` itself.** Its one mechanical checkpoint is the JavaScript self-check in Step 6, which runs over the reviewer's parsed block and asserts nothing about `workflow_steps` at all; the completion checklist separately requires the six entries to be present, but says nothing about the keys *within* an entry. So the only validation this key ever meets is the server's. There, `valid_step?/1` checks `name`, `dispatched`, `duration_ms` and `reason`, and separately the `reason_code` enum — but **not** `dispatch_count`. Unlike `reason_code`, which is rejected when unrecognised, this key is accepted whatever its type, persisted into the `workflow_steps` array, and returned verbatim by the API. Nothing reads it yet, so the cost today is a corrupt record rather than anything worse. **Whatever reads it first — a renderer, a dashboard, an aggregation — has to guard for a non-integer itself, unless the validator first gains the optional-but-validated shape `reason_code` already has.** Recorded here so the next person adding a key does not inherit the absence as though it were a design choice.

**Why not simply record tokens.** The obvious fix for (1) is a token count, the one figure observed to rank correctly. The task that introduced these keys ruled that out — *record what is actually measurable* — and that stands. But be exact about the reason, because the usual one is wrong: some runtimes **do** measure a per-dispatch token cost, so recording it would not be inventing it. The unsettled question is **portability**, whether every runtime this schema serves can produce one. Until that is answered, this pair is what exists, read within the limits above.
