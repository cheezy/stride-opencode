...
The escalation policy above applies **only** on the path where a session actually ran and returned a Critical finding — it changes nothing about these three fallback cases. **No exploratory finding can block completion on a task that never ran a session.**
