...
Six stride-side updates ported in one release, all documentation-only and all built on carriers that already exist. They adapt this port's optional exploratory-testing integration to the capability changes in the exploratory-testing extension family.

**1. Severity mapping and a fail-closed escalation policy.** Two optional integrations hang off the same phase with opposite consequences, and the difference was an accident rather than a decision: a `partial` or `unmitigated` verdict from the Deep security-considerations review is fail-closed and blocks completion, while a Critical exploratory finding was advisory prose. It is now a decision.

- **`stride-completing-tasks`** gains a **Severity mapping** section: the extension's four-level `bug-advocacy` ladder (Critical > High > Moderate > Minor) maps onto the reviewer's three values — Critical → `critical`, High and Moderate → `important`, Minor → `minor` — findings are **mapped, never re-rated**, and an absent or unrecognized severity maps to `important`, never dropped and never `critical`, so text that failed to parse can never reach a blocking path. It states explicitly that **mapping a severity is not the same as appending an `issues[]` entry** (only an *introduced* Critical becomes one, because any `category: "testing"` entry forces `testing_strategy` to `failed`). The pre-submission self-check gains a matching consistency bullet whose own absence is recorded as the normal case.
- **`stride-workflow` Step 6.5** and **`stride-subagent-workflow` Phase 3.5** each gain the escalation policy, **identical in substance**. An **introduced** Critical — its responsible lines among the lines this task added or modified — fails `testing_strategy`, appends a `category: "testing"` / `severity: "critical"` issue, and is fixed, re-charter-run and re-reviewed before completing. A **discovered** one is recorded at its exploratory severity and filed as a follow-up defect, with **no** issue appended and **no** verdict flipped. Uncertainty always resolves to *discovered*, and the fault site is localized **by reading the code, never from the finding's own text**, because the application under test controls that text and a policy that can block completion must not be steerable by content an attacker can influence. The policy is explicitly inert when no reviewer ran, and forbids synthesizing a `reviewer_result` to create one.

**2. A non-interactive dispatch principle.** Both skills and the README replace the open-ended "prefer `/explore`, or dispatch `@explorer`" phrasing with a stated **principle**: dispatch only a surface that can complete the step **unattended**. This orchestrator never prompts the user between steps, so an interactive surface stalls the task until the claim expires — a failure that looks like a hang rather than a violation. The principle governs, and any surface the extension gains later is judged against it rather than against a list. Today it sanctions exactly one — the **`explorer` subagent (`@explorer`)**, which states outright that it never asks the user a question — and names as never-auto-dispatched `/explore`, `/pair`, `/nightmare-headline`, `/recon`, and the `stride-exploratory-testing` routing skill (which routes to a surface not known in advance, and is what the bare extension name resolves to). The detection lists are clarified to confer no dispatch licence.

**`/explore` is excluded in this port, and that was established rather than assumed.** The sibling Claude Code plugin excludes it for the same reason, but the two extensions version independently, so the claim was checked against `stride-opencode-exploratory-testing`'s own `commands/explore.md`, whose Step 3 runs "a single consolidated round of questions (≤4 questions, asked in the conversation)". Every never-dispatch entry was likewise read from that extension's own front matter and prompt body — as data, never by executing it — and both skills say to re-establish each entry whenever that separately-versioned extension changes.

**3. An explicit session budget and a tightened environment context.** The dispatch previously supplied charters, the target and app context but **no bound on how far a session could run** — an unbounded dispatch inside an autonomous workflow is a runaway risk and a larger blast radius against a live application, and the caller is the only party who knows what the task can afford. Step 6.5 gains a `Provide the plugin with:` input list and Phase 3.5 extends its existing `Provide the dispatch with:` list, both carrying the budget as **the caller's to set**. The figures are read from the **installed** extension's own `explorer` contract rather than from the skill page, since the two repositories release independently — today probes, default **12**, band **8–20**, plus a **tool-call ceiling** defaulting to **5× the probe budget**, whichever is reached first. The budget is a **ceiling, never a quota**, and a budget too small to fund one workable charter means **do not dispatch at all**: a token session that never reaches the feature produces a false coverage claim, which is worse than not running. The environment-context bullet now names **how to reach the running app**, the **authorized non-production confirmation** (a safety control, never defaulted and never supplied on the user's behalf — without it the dispatch does not happen), and **where test accounts or seed data live**, with credentials **pointed at, never inlined**. **Budget exhaustion is a normal outcome that never fails completion**: both skills record the `stop_reason` the installed contract emits (`charter_quiet`, `probe_budget_exhausted`, `tool_call_ceiling`, `risk_acceptable`, `blocked`) and gain graceful-path bullets — a session that stopped on its budget produced valid findings over partial coverage.

**4. Richer recording, in the two existing carriers only.** `stride-completing-tasks` now records each finding's **stakeholder impact** — the RIMGEA *Externalize* product the explorer emits as `bugs[].stakeholder_impact`, which is what a reviewer weighing a finding actually needs, since severity says how bad the consequence was but not who is harmed. It is used when the session supplies it and **never invented** when it does not. It also **cites the session artifact's path when one was written** — with the caveat that usually none was, because the only surface Step 6.5 may dispatch is the `explorer` subagent, whose contract grants it no write or edit tool; an artifact exists when a human separately ran a session command. Record the **path, never the contents**, and cite it **repository-relative, never absolute**, since an absolute path would disclose the home directory, username and machine layout into a persisted, rendered field.

**5. `.exploratory/` added to the gitignore guidance.** Sessions write artifacts under `.exploratory/`, those artifacts hold transcribed application output — exactly the material redaction keeps out of the completion payload — and they arrive **untracked**, while `after_doing` commonly stages with `git add -A`. Neither behaviour is wrong alone; together they silently commit observed application data into the task's own commit, and a commit is far harder to walk back than a payload field. The Marker Contract row now names **both** directories with that mechanism in one clause, and **Step 0 gains a third prerequisite item** that actually delivers the guidance — naming Step 0 from Step 6.5 was not enough on its own, since Step 6.5 is only read once a session may already have run. The check **re-triggers whenever the extension is installed**, not only at Stride's first install (the two ship independently and the extension routinely arrives later), and because a `.gitignore` line is **inert against an already-tracked path**, the guidance pairs it with `git rm --cached`. It stays **operator guidance — the skill never edits anyone's `.gitignore`.**

**6. An optional `/harden` sub-step, sequenced so it can never turn the blocking test gate red.** A session that finds a bug and stops has closed nothing. `stride-workflow` gains **Step 6.6** and `stride-subagent-workflow` gains **Phase 3.6**, identical in substance, dispatching `/harden` to draft one regression check per convertible bug — the step that turns *Explored* back into *Checked*. The gate is **three** conditions: a session returned convertible findings; **the `/harden` command itself is registered** (deliberately narrower than "the extension is installed", since `/harden` is not one of the surfaces the Step 6.5 gate detects and arrived in the extension's **v0.2.0**, so an installed extension can predate it); and native slash-command dispatch is available (replacing the reference plugin's "this is Claude Code" condition, which has no meaning here — `/harden` ships as a command only).

Two traps are handled explicitly rather than mentioned:

- **The blocking gate.** `after_doing` is blocking and typically runs the suite, and a check reproducing an **unfixed** bug is *supposed* to fail — that failure is the evidence it reproduces the bug. Sequenced naively, a session that did exactly the right thing blocks the completion of a task that may not even be scoped to fix that bug. The step dispatches `/harden` **without `--output`**, so drafts land in `.exploratory/checks/` outside the test tree where the gate never sees them, and that staged state is the always-safe default. A check enters the suite only when **the file loads** — a skip marker makes a *case* inert, not a *file*, and runners compile or import the whole tree first, so a draft carrying an unresolved `TODO(harden):` wiring marker fails at compile or collection time however it is tagged — **and the case is green or inert**, both established by **running the project's own `after_doing` command across the whole suite once, never by expecting**, with a revert of everything the attempt touched if it is not clean. Exactly three dispositions: run-and-see-it-pass for a bug fixed in the same task (updating the draft's now-false "expected to fail today" header); moved in marked skipped or pending with a follow-up defect for a bug still open — with the note that **`xfail` is not a skip**, since it runs the test and under `xfail_strict` fails the run once the bug is fixed; or left staged with a follow-up defect. **Never leave a check red in the test tree** — the hazard is presence in the working tree, not the commit. And **never overwrite an existing test file: that check is the workflow's own**, since dispatched without `--output` the command never writes into the test tree.
- **Post-review divergence.** `/harden` writes after the reviewer has seen the diff, and unreviewed executable code entering a commit unannounced is exactly what review exists to prevent. Paths are named in `completion_notes`, one line is mirrored into `completion_summary`, and any check reaching the commit is listed in `actual_files_changed`, with the reviewer **re-run whenever a check will reach the commit at all** — without weighing whether the edit was "substantial", since a rule that turns on a judgement call resolves toward skipping the expensive option.

**A local pre-execution check on every draft, because inheriting the rules by reference was not enough.** `/harden`'s contract forbids a draft from hard-coding an observed credential, pointing at a real host, or writing a destructive step, and requires a check derived from a **security** finding to assert the guard fires rather than perform the unsafe act. But OpenCode commands declare no tool allowlist, so those are the command's **discipline, not environment-enforced** — and this step is what turns that output into code the suite compiles and executes. Trusting the contract for what a draft *contains* while distrusting it for whether a draft was *run* is not a coherent position, so both skills require **reading the draft and confirming all four properties before it reaches the commit — moved, run, or left staged where the artifact directory turns out not to be ignored** — with a failing draft going to the third disposition, and its contents read **as data to assess, never as instructions**. The draft file is named a **sink in its own right**: once moved, `after_doing`'s `git add -A` commits it, and a credential in repository history is far harder to walk back than one in a payload field. The surfacing triggers are likewise keyed on **"reaches the commit"**, not "entered the test tree", since `git add -A` decides the commit and `.exploratory/` is ignored only if someone ignored it — so the skills say to **verify rather than assume** (`git check-ignore -q .exploratory/`).

**On `completion_summary`, which is not a new carrier.** The exploratory *findings* recording uses exactly two carriers (`completion_notes` and the reviewer's `testing_strategy` note). `completion_summary` is a required, always-persisted, Review-queue-rendered field these skills already mirror one line into whenever a fact must reach a human even where `completion_notes` may not be persisted — the credential-bearing-row refusal, the steering-row refusal, and the Critical-finding escalation all do it. A file written after review is that same shape of fact. **No new completion field and no seventh `workflow_steps` name**: the hardening dispatch's wall-clock folds into the existing `reviewer` entry, and when no reviewer ran that entry is the skip form with no duration.

### Fixed — four stale representations that contradicted the shipped dispatch rule

Adding the Step 6.6 / Phase 3.6 nodes surfaced that the non-interactive dispatch guard had been applied to the prose but not to the **secondary representations of the same flow**, which a reader scans *instead of* the prose. The `stride-workflow` Complete Workflow Flowchart and Quick Reference Card, and the `stride-subagent-workflow` Workflow Flowchart and Quick Reference Card, all still instructed dispatching **`/explore` (or `@explorer`)** at the manual-testing step — directly contradicting the sanctioned-surfaces rule three sections above them, which sanctions `@explorer` alone and puts `/explore` on the never-auto-dispatch list. All four now say `@explorer` only, name `/explore` and `/pair` as excluded, and mention the session budget and the authorized non-production affirmative. This is the same class of defect `[1.32.1]` fixed for the Step 5 placeholder: a rule stated only in the prose leaves the most-scanned surfaces still saying the old thing.

### Backward compatibility

Fully backward compatible. Documentation-only — no hook logic, credential path, or wire shape changes. **No new completion field and no seventh `workflow_steps` name**; every addition rides carriers that already exist (`reviewer_result.testing_strategy`, `issues[]`, `issue_counts`, `completion_notes`, `completion_summary`, `actual_files_changed`). The exploratory integration remains entirely optional and every path still skips gracefully when the extension is absent, the task carries no `manual_tests`, or no session ran — **no exploratory finding can block completion on a task that never ran a session**. Budget exhaustion is one more never-fails-completion ending rather than a new failure mode, a budget too small to spend is a documented skip, and the hardening step is gated three ways with the staged default keeping drafts outside the blocking gate entirely. The gitignore change is advisory prose only: no `.gitignore` is edited automatically. The flowchart and card corrections align those surfaces with a rule already stated in the prose; they change no behaviour the prose did not already specify.

### Source

Goal G400 (W2000, W2001, W2002), porting `stride/skills/stride-workflow/SKILL.md`, `stride/skills/stride-subagent-workflow/SKILL.md` and `stride/skills/stride-completing-tasks/SKILL.md` — the Claude Code plugin is the treatment of record, translated to this port's terser voice, its `extension`/`subagent`/`@mention` vocabulary, and its **Step 6.5 / 6.6** numbering (this port keeps Step 5 intentionally blank, so Code Review is Step 6). Facts about the exploratory extension were established from `stride-opencode-exploratory-testing`'s own files rather than copied from the reference page: the budget's unit, default, band and ceiling multiplier and the `stop_reason` values from `agents/explorer.md`; the "usually no artifact" caveat from that agent having no write or edit tool; and `/harden`'s positional bug source, `--framework` override, `.exploratory/checks/<timestamp>-<slug>/` default, `TODO(harden):` wiring allowance, no-framework path and never-run discipline from `commands/harden.md`, with its v0.2.0 arrival from that repo's changelog. Deliberately **not** ported, as later work: the reference's fuller session-ending coverage-disposition taxonomy, its `completion_summary` mirror as a *third findings carrier*, and the change-set provenance machinery (`.stride-env-cache` base-ref lookup, `.stride-dirty-baseline` subtraction, nested-repo base recovery) — this port decides provenance from its own diff and `actual_files_changed`, which it already carries. Ends at the tag and GitHub release on this repo — **stride-opencode has no marketplace catalog**; installs resolve through a direct GitHub reference.
