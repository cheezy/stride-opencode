...
8. **Return Structured Review**:
   ...
   - The JSON object has these top-level fields (all required unless explicitly marked OPTIONAL, snake_case throughout):
     - `schema_version`: string. Always `"1.7"` for this prompt version. (Bumped from `"1.6"` by W2165, which added the `cosmetic` key to `issues[]` entries — prior additive changes held at `"1.6"` precisely because they added no field.)
     ...
     **`cosmetic` — presentational only, and it changes disposition, never the finding.** Set it `true` only when **both** claims hold: the *finding's* claim is correct, **and** the *artifact* it points at asserts nothing that is itself false. That second gate is the one that is easy to miss, because a finding about a false statement is itself perfectly correct — a document that says "prints six values" where seven are printed, a comment saying an assert runs below a write when it sits above, a heading reading "Two limits" above three. In each the finding is right, which satisfies the first gate; what makes them **substantive** is that the artifact states something false. **A false statement of fact is never cosmetic**, however small, and however much its subject resembles a count or a wording. With both gates satisfied, the subject must then be purely presentational: line wrapping, column width, word or line counts, phrasing preference, ordering of otherwise-equivalent prose. **Read that list as examples of a test, never as a list to match against.** The same edit can be either: a re-wrap of a paragraph is cosmetic, while a re-wrap inside executable content — a jq expression, a shell heredoc, a fenced block something greps or extracts — changes what runs and is **substantive**. So is a re-ordering in a document whose structure is load-bearing. Apply the test, not the examples. **The default direction is the safe one:** absent means `false`, so an unflagged finding buys a round; when you are unsure whether a finding is presentational, omit the flag.

     **What it does NOT do — four clauses, each a way the flag could be abused into a suppression.** It does **not** change `severity`: a cosmetic finding is `minor` because it is presentational, and carries the same honest severity it would have carried without the flag. It does **not** change `category`: a cosmetic `code_quality` finding stays `code_quality`. It does **not** change `status`: `status` is driven by `critical`/`important` issues and by `not_met` criteria and checks, none of which a `minor` touches, **so the `status` rule above is unaffected and needs no exclusion added to it** — the flag is approval-neutral and NEVER contributes to `changes_requested`. And it does **not** remove the finding from `issues[]`, from your report, or from anything downstream; `issue_counts` still counts it in its `minor` bucket. **The single thing it changes is the orchestrator's re-review disposition.** That is why it is not a carve-out, an exclusion or a suppression, and why it needs no amendment to any exhaustive-carve-out wording elsewhere in this file: it removes no finding from any record and excuses no verdict.

     **It is never a way to downgrade a real finding.** `cosmetic: true` on a finding whose claim is *substantive* — anything where the code or contract would behave differently, or a reader would be misled — is a **reviewer defect**, not a judgement call. The test is not "is this small" but "is the only thing wrong here how it looks". A one-character typo that inverts a meaning is substantive; a line four columns too wide is cosmetic. **`minor` and `cosmetic` are orthogonal, not synonyms:** a `minor` finding can be substantive (a real but low-impact defect), and those still buy a round. This is the same prohibition the round-scoping note above already states from the other side, where the fixes list "never licenses downgrading a severity".

     **Three conditions are prohibited, and in this port the prohibition is prose you follow rather than something the port evaluates.** `cosmetic: true` is wrong when the severity is anything other than `minor` — which covers **`critical` and `important` alike** — when `category` is `"security"`, or when the value is not a real boolean (no coercion: `1`, `"yes"` and `"true"` are all wrong; a missing key, not a falsy one, is how you say "not cosmetic"). **Say what this port does and does not check.** Its only mechanical checkpoint on a review is the JavaScript self-check under "Extracting the structured review block" in `stride-workflow` Step 6, which asserts that no top-level key the reviewer emitted was dropped by the copy, that the `project_checks` count matches, and that the acceptance-criteria count matches — it **never inspects an `issues[]` entry's keys**. stride pins these three conditions as a shape check on two extraction paths this port does not carry. What *is* mechanically verified is that this rule is present at all: the `canon:cosmetic-finding-class` anchor above is read by `stride/scripts/check-port-canon.sh`, which reports this port MISSING until it lands and STALE if the canon's version moves past it. **It tests the anchor, never the sentence.** Disclosed here rather than left implied.

     **On the security exclusion, stated rather than left to inference.** It is belt-and-braces for review step 5's findings, and **load-bearing for the two mandated sites named at the end of this paragraph, whose severity this contract does not pin** — so do not read it as redundant and drop it. For step 5's findings it is belt-and-braces: review step 5 flags an unaddressed consideration **Important** and an exploitable one **Critical**, so step 5 never produces a `minor` security finding, and the severity restriction already excludes every finding it can emit. **Name the residual plainly:** a security-relevant observation the reviewer files under `code_quality` (or `pattern`, `testing`, `project_check`) rather than `security` is **not** reached by a category-shaped exclusion, and review step 6 flags issues as Minor unless they could cause runtime failures — so a misfiled security nit is legitimately a `minor` and is cosmetic-eligible. That is accepted rather than closed, because excluding `category: "security"` reuses the boundary this port already relies on twice for exactly this class of decision, in `stride-workflow` Step 6; inventing a second, wider boundary here would leave two rules in this port disagreeing about what a security finding is. The remedy for a misfiled finding is the filing. Two mandated sites the flag must therefore never reach are both `category: "security"` and so are already covered: the `considerations[]` fail-closed escalation below, and the credential carve-out in review step 4.

     **Canon-governed — entry `cosmetic-finding-class` in `stride/docs/port-canon.md`.** That entry registers these paragraphs as one rule every port must carry. A change to their substance owes a version bump in **two** places before the next release: that entry in the canon, and this file's own `<!-- canon:cosmetic-finding-class ... -->` anchor above.
     ...
     The two rules above are shared, not local — they are entry `verdict-note` of `stride/docs/port-canon.md`, and that is where their substance is held for every port. **Rewriting the substance here owes two version bumps in the same change**: one on that canon entry, one on the anchor beside it.
